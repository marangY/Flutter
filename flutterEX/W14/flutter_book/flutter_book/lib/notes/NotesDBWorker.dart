...
class NotesDBWorker {
  ...
  Future<Database> init() async {
    ...
    String path = join(utils.docsDir!.path, "notes.db");  // path의 null check로 ! 추가
  ...
  Future create(Note inNote) async {
    ...
    List<Map> rawList = await db.rawQuery("SELECT MAX(id) + 1 AS id FROM notes");
    int id = -1;
    if (rawList.first["id"] == null) id = 1;
    else id = rawList.first["id"] as int;
    print("create record : id = $id");
    // Insert into table.
